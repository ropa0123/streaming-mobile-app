...
class TechHubApp(MDApp):
    def build(self):
        ...
        # Initialize Android player objects if on Android
        if platform == 'android':
    ...
    def toggle_stream(self):
        ...
        if self.player is None:
            try:
                if platform == 'android':
                    self.player = self.AndroidMediaPlayer()
                    self.player.setDataSource(url)
                    # Set audio attributes for streaming
                    att_builder = self.AudioAttributesBuilder()
                    att_builder.setUsage(1) # USAGE_MEDIA
                    att_builder.setContentType(2) # CONTENT_TYPE_MUSIC
                    self.player.setAudioAttributes(att_builder.build())
                    
                    self.player.prepareAsync()
                    # We start playing when prepared
                    self.player.setOnPreparedListener(self._on_android_prepared)
                else:
                    # Fallback for desktop testing - requires ffpyplayer installed locally
                    try:
                        from ffpyplayer.player import MediaPlayer as DesktopPlayer
                        self.player = DesktopPlayer(url)
                    except ImportError:
                        toast("ffpyplayer not found for desktop playback")
                        return
                
                btn.text = "STOP LISTENING"
                icon.text_color = [0, 1, 0, 1]
            except Exception as e: 
                toast(f"Stream Error: {str(e)}")
        else:
            self.stop_stream()

    def _on_android_prepared(self, mp):
        mp.start()
